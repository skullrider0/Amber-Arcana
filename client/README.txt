...
This revision keeps the 0.1.8 gameplay manifest and applies conservative performance tuning.
See OPTIMIZATION-0.1.9.txt for the active settings, rollback values, and profiling notes.

Amber & Arcana — 0.1.8 alpha
...
CLIENT FIX 1:
- Removed Local Looks (locallooks-1.5-1.20.jar / CurseForge file 4576728) from the client manifest because the dedicated server intentionally excludes this client-only mod and Forge rejects the connection when it is present only on the client.

0.1.9-alpha-r3 content sync:
- Added Just Dire Things [Forge] 1.1.7 (CurseForge project 1533501, file 8733229).
- Added DecoCraft 3.0.4 for Forge 1.20.1 (CurseForge project 79616, file 7092533).
- Both are required on client and dedicated server.
...
- CurseForge manifest recommendedRam is set to 10240 MB (10 GiB), so CurseForge can apply the pack author memory recommendation instead of its 4096 MB default.


0.1.9-5 PACKET + SESSION FIX
- Added Packet Fixer 2.0.0 to the client. Matching dedicated server build also contains it.
- Added NeoAuth 1.20.1-1.0.3 to the client for in-game Minecraft session re-validation.
- NeoAuth is client-only; it is intentionally omitted from the dedicated server.
- Old ReAuth was not used because it does not provide a Forge 1.20.1 build.
