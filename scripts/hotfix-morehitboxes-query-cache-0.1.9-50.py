...
def patch_metadata(manifest_count: int, server_count: int, sha512: str) -> None:
    ...
    summary["more_hitboxes_query_cache_0_1_9_50"] = marker(sha512)
    ...
    for rel in (
        ...
        data["more_hitboxes_query_cache_0_1_9_50"] = marker(sha512)
        dump(path, data)
...
def patch_readme() -> None:
    ...
    note = """### 0.1.9-50 More Hitboxes query-cache performance patch
...
"""
    if "### 0.1.9-50 More Hitboxes query-cache performance patch" not in text:
        text = text.rstrip() + "\n\n" + note
    path.write_text(text, encoding="utf-8")
...
def patch_changelog() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    if "## 0.1.9-50" not in text:
        entry = """## 0.1.9-50 — More Hitboxes query-cache performance patch
